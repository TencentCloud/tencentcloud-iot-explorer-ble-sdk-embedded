...
static int ble_property_brightness_set(const char *data, uint16_t len)
{
    ...
    if ((brightness_value < BLE_QIOT_PROPERTY_BRIGHTNESS_MIN) || (brightness_value > BLE_QIOT_PROPERTY_BRIGHTNESS_MAX)){
...
static char sg_test_name[32 + 1] = "default name";
...
int ble_event_get_data_by_id(uint8_t event_id, uint8_t param_id, char *out_buf, uint16_t buf_len)
{
    ...
    if (ret_len < 0) {
        ...
        if (ble_check_ret_value_by_type(sg_ble_event_array[event_id].event_array[param_id].type, buf_len, ret_len)){
            ...
            ble_qiot_log_e("evnet data length invalid, event id %d, param id %d, type %d", event_id, param_id, sg_ble_event_array[event_id].event_array[param_id].type);
            return -1;
        }
    }
}
...
static int ble_action_handle_loop_input_cb(e_ble_tlv *input_param_array, uint8_t input_array_size, uint8_t *output_id_array)
{
    int result = 0;
...
static ble_action_t sg_ble_action_array[BLE_QIOT_ACTION_ID_BUTT] = {
        {ble_action_handle_loop_input_cb, ble_action_handle_loop_output_cb,
                sg_ble_action_loop_input_type_array, sg_ble_action_loop_output_type_array,
                sizeof(sg_ble_action_loop_input_type_array) / sizeof(uint8_t),
